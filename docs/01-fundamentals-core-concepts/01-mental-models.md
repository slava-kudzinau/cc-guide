...
For detailed CLI usage, transport types, and scope management, see [Section 2.4: MCP Server Configuration](../01-fundamentals-core-concepts/02-environment-setup#24-mcp-server-configuration).
...
> **Note:** The JSON examples above show the configuration structure generated by CLI commands. For most use cases, use `claude mcp add` rather than editing JSON files directly. See [Section 2.4](../01-fundamentals-core-concepts/02-environment-setup#24-mcp-server-configuration) for complete CLI documentation.
